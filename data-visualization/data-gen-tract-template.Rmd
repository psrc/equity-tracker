...
# store the pivot tables in a list & iterate thru list and calculate for Regional numbers
data_equity_pivot_dfs <- map(paste0("data_equity_pivot", str_extract(years, "\\d{2}$")), ~get(.x))
data_equity_pivot_calc_dfs <- map2(data_equity_pivot_dfs, years, ~equity_pivot_calc(.x, .y))

# name elements in lists consistently
names(data_equity_pivot_dfs) <- names(data_equity_pivot_calc_dfs) <- paste0('yr',as.character(years))

```

```{r}
# check data sets
walk2(years, data_equity_pivot_calc_dfs, ~print(paste0("Number of rows ", .x, ": ", nrow(.y))))
...
sum(tract.10$estimate)
sum(tracts20.lyr$total_pop20)
...
walk2(years, data_equity_wt_dfs, ~print(paste0("Number of rows ", .x,": ", nrow(.y))))
```
