...
# Download data 
## Primary data 
Data comes from the [First Street Foundation Initiative](https://riskfactor.com/methodology/flood). Data are available (manual download) for 2022-2052.
```{r, include=FALSE}
folder_path <- "Y:/Equity Indicators/tracker-webpage-content/c-climate-change/c01-flood-risk/raw-data"
flood_22 <- read.csv(file.path(folder_path,"fsf_flood.csv"))

indicator_measure <- "average"
```
\
## Additional data
...
* low, low medium, etc.) at the tract-level for each equity focus group for the years that correspond to the flood risk score (2022)
```{r, include=FALSE}
...
### tract-level population data for 2021 - not used for this indicator
```{r, include=FALSE}
...
tracts20.lyr<-tracts20.lyr[,-20] #removing the 2020 pop column
...
# Explore data
The 2022 flood risk score data are available in 2020 geographies.
```{r, include=FALSE}
...
```{r, include=FALSE}
# join geographic data to flood risk score data
data_tract <- merge(tracts20.lyr, flood_22,
                    by.x="geoid20",
                    by.y="Census.Tract", 
                    all.x=TRUE)
...
data_tract <- merge(x = data_tract, y = tract.21[ , c("geoid20", "total_pop21")], by = "geoid20", all.x=TRUE)
...
# set the variable
var_name <- "Average Flood Risk Score"
...
# based on the plot in the last chunk, this code is meant to isolate outlier(s) - in this case it filters for the outliers with high risk scores and flood risk score
...
county_tract_data <- data_tract %>% 
  ...
  dplyr::mutate(cnty_estimate=(sum(estimate*prop_w_score, na.rm =TRUE))/(sum(prop_w_score, na.rm=TRUE))) %>% #calculate weighted value by county using number of properties with a flood score
  dplyr::distinct(county_name, cnty_estimate)

# calculating data by region
reg_tract_data <- data_tract %>% 
  st_drop_geometry() %>%  #to remove spatial component
  dplyr::select(estimate, prop_w_score) %>% #to simplify table to necessary fields
  dplyr::mutate(reg_estimate=(sum(estimate*prop_w_score, na.rm =TRUE))/(sum(prop_w_score, na.rm=TRUE))) %>% #calculate weighted value for region
...
data_equity_tracts <- merge(flood_22, equity_tracts_21,
                            by.x="geoid",
                            by.y="geoid", 
                            all.x=TRUE)

# join 2020 acs population data to flood risk score data - this is for eventual weighting (which may not be necessary depending on the indicator)
...
data_equity_quintiles21 <- data_equity_tract21 %>% 
  ...
  dplyr::mutate(average_num=as.numeric(average))
...
## Calculations
### Calculate by region
The flood risk score are available at the census tract level, but we want to calculate the regional average, using the census bureau's tract-level population estimates (2021).
```{r}
data_equity_pivot21_calc <- data_equity_pivot21 %>%
  filter(!is.na(quintile)) %>%
  dplyr::group_by(equity_group, quintile, county) %>%
  summarise(wt_average=(sum(average_num*prop_w_score, na.rm =TRUE))/(sum(prop_w_score, na.rm=TRUE))) %>%
  as_tibble() %>%
...
### Calculate by equity quintile - already calculated in data_equity_pivot21_calc
The scores are available at the census tract level but we want to calculate the weighted values by the 6 equity/5 quintile groups, using the census bureau's tract-level population estimates (2020). 

```{r}

# calculate 2021 region population
# population_test <- data_equity_pivot21 %>%
#   distinct(GEOID, .keep_all = TRUE) %>%
#   dplyr::summarise(pop=formatC((sum(estimate, na.rm=TRUE)), format="d", big.mark=",")) #4,197,443

# sum(tracts20.lyr$total_pop21) 

# calculations to get weighted average for flood risk score per equity group/quintile
# data_equity_wt21 <- data_equity_pivot21 %>%
#   filter(!is.na(quintile)) %>% # necessary because of added na columns from pivot
#   dplyr::mutate(tot_risk_tract=prop_w_score*average_num) %>%
...
#                    tot_prop=(sum(prop_w_score, na.rm = TRUE))) %>%
#   dplyr::mutate(wt_average=tot_rate/tot_prop) 

# check population
# data_equity_wt21_test <- data_equity_wt21 %>%
#   dplyr::filter(equity_group=="income_quintile")
#   sum(data_equity_wt21_test$tot_prop) #4,197,443
...
low_lowmed_years <- unique(low_lowmed$data_year) #2021
...
data_tract <- rbind(low_lowmed, #2
                    other_data) #148

nrow(data_tract) #150 - should be fewer because of duplicates (148)- low and low medium present in data set because of aggregation/calculations in previous steps - need to be removed and replaced with averaged values
...
data_clean <- data_clean %>%
  dplyr::mutate(estimate=as.numeric(wt_average)) #rename value variable for consistency
