...
# summary(data_tract$variable_value) # max is 100.48, so rounding to 100 makes sense
...
#  53033000500 (Broadiew neighborhood, King), 53035091001 (Bainbridge, Kitsap), 53053072409 (Artondale Pierce), 53035091002 (Bainbridge, Kitsap), 53033005600 (Briarcliff and Magnolia neighborhoods, King),53033012100 (Arroyo Heights and Arroyo Beach neighborhoods, King), 

sort <- data %>% 
  dplyr::arrange(variable_value) %>% 
  select(county_name, geoid20, variable_value, reg_estimate, cnty_estimate)
# 53033005304 (UW campus, King County), 53033005303 (UW campus, King County), 53053072903 (Joint Base Lewis-McChord)
```

1. 61%: The region's voter participation rate in 2024
...
* There are six census tracts that have voter participation rates of 95% or higher - these include three neighborhoods in Seattle in King County: Broadiew, the Briarcliff and Magnolia neighborhoods, and Arroyo Heights and Arroyo Beach, two neighborhoods on Bainbridge Island in Kitsap County, and one neighborhood in Artondale, Pierce County.
* The three census tracts that have the lowest voter participation rates in the region (below 10%) are in King and Pierce counties - around the UW campus in Seattle and surrounding Joint Base Lewis-McChord including Fort Lewis and North Fort Lewis, respectively. These areas have high concentrations of students and military members - those who are less likely to be registered as voters of Washington state.  
...
# region low quintile poc households: 0.7800451		
# region high quintile poc households: 0.4362275
# 0.7800451-0.4362275=0.3438176, 0.34 when rounded
...
filter <- data_clean_column %>% 
  filter(county=="Region",
         ...
         quintile=="Low"|
           quintile=="High")
# region dif bt low/high quintiles for disability: 0.6135849-0.5491283=0.0644566, 0.06 when rounded
...
# king low-high quintiles for poc: 0.7976208-0.4176189=0.3800019, 0.38 when rounded
# kitsap low-high quintiles for poc: 0.8149331-0.4845954=0.3303377, 0.33 when rounded
# pierce low-high quintiles for poc: 0.7764473-0.4094535=0.3669938, 0.37 when rounded
# snohomish low-high quintiles for poc: 0.7498939-0.5060756=0.2438183, 0.24 when rounded
...
# king high quintile low income households: 0.4202010
# kitsap high quintile low income households: 0.5361186
# pierce high quintile low income households: 0.3782806
# snohomish high quintile low income households: 0.4969133
...
# region low-high quintiles for lep: 0.7200263-0.4539109=0.2661154, 0.27 when rounded
# king low-high quintiles for lep: 0.7534495-0.4520894=0.3013601, 0.30 when rounded
# kitsap low-high quintiles for lep: 0.7087535-0.6364957=0.0722578, 0.07 when rounded
# pierce low-high quintiles for lep: 0.6795247-0.4639213=0.2156034, 0.22 when rounded
# snohomish low-high quintiles for lep: 0.7314116-0.4926553=0.2387563, 0.24 when rounded 
...
filter <- data_clean_column %>% 
  filter(equity_group=="older_quintile",
         quintile=="Low"|
           quintile=="High",
         county!="Region")
# king high-low quintiles for older: 0.6728023-0.4759738=0.1968285, 0.20 when rounded
# kitsap high-low quintiles for older: 0.7830916-0.4808821=0.3022095, 0.30 when rounded
# pierce high-low quintiles for older: 0.7203634-0.4443613=0.2760021, 0.28 when rounded
# snohomish high-low quintiles for older: 0.7233040-0.5549716=0.1683324, 0.17 when rounded 
```
\

#### Insights & Analysis

* Communities with higher concentrations of people of color have lower voter participation rates than those with lower concentrations - the largest difference is in King County (38 percentage points), followed by Pierce (37 percentage points), Kitsap (33 percentage points), and Snohomish (24 percentage points).
* For communities with high concentrations of households who are below 200% of the poverty level, voter participation rates are highest in Kitsap County (54%), followed by Snohomish (50%), King (42%), and Pierce County (38%).
* Voter participation is lower in communities with higher concentrations of households with limited English proficiency compared to communities with lower concentrations. King County has the highest difference (30 percentage points), while Kitsap County has the smallest difference (7 percentage points).
...
# 2024 region low quintile poc households: 0.7800451
# 2024 region high quintile poc households: 0.4362275
# 0.7800451-0.4362275=0.3438176, 0.34 when rounded
...
# King low-high poc 2024: 0.7976208-0.4176189=0.3800019, 0.38 when rounded
# ((0.38-0.34)/(0.34))*100: 11.76471% change
# Kitsap low-high poc 2012: 0.8359785-0.6344333=0.2015452, 0.21 when rounded
# Kitsap low-high poc 2024: 0.8149331-0.4845954=0.3303377, 0.33 when rounded
# ((0.33-0.21)/(0.21))*100: 57.14286% change
# Pierce low-high poc 2012: 0.8335060-0.5705603=0.2629457, 0.26 when rounded
# Pierce low-high poc 2024: 0.7764473-0.4094535=0.3669938, 0.37 when rounded
# ((0.37-0.26)/(0.26))*100: 42.30769% change
# Snohomish low-high poc 2012: 0.7012653-0.4730731=0.2281922, 0.23 when rounded
# Snohomish low-high poc 2024: 0.7498939-0.5060756=0.2438183, 0.24 when rounded
# ((0.24-0.23)/(0.23))*100: 4.347826% change
...
# 2024 king low-high quintile income: 0.7335528-0.4202010=0.3133518
# 0.33-0.31=0.02
# 2012 kitsap low-high quintile income: 0.8426371-0.5829897=0.2596474
# 2024 kitsap low-high quintile income: 0.7919757-0.5361186=0.2558571
# 0.26-0.26=0
# 2012 pierce low-high quintile income: 0.8531638-0.5400349=0.3131289, 0.31 when rounded
# 2024 pierce low-high quintile income: 0.7374310-0.3782806=0.3591504, 0.36 when rounded
# 0.31-0.36=-0.05
# 2012 snohomish low-high quintile income: 0.7074280-0.4330401=0.2743879, 0.28 when rounded
# 2024 snohomish low-high quintile income: 0.6940892-0.4969133=0.1971759, 0.20 when rounded
# 0.28-0.20=0.08
...
# 2024 high-low older: 0.7121481-0.4839383=0.2282098, 0.23 when rounded
