...
# Download data 
## Primary data 
Data comes from [WTN](https://fortress.wa.gov/doh/wtn/WTNPortal#!q0=821). Separate CSVs at the census tract geography are available (manual download) for 5y time spans at 1y increments, starting in 2000, until 2020.
```{r, include=FALSE}
...
cardio_mort_06_10 <- read.csv(file.path(folder_path,"Cardiovascular_Disease_Mortality_06_10.csv"))
cardio_mort_11_15 <- read.csv(file.path(folder_path,"Cardiovascular_Disease_Mortality_11_15.csv"))
cardio_mort_16_20 <- read.csv(file.path(folder_path,"Cardiovascular_Disease_Mortality_16_20.csv"))
```
\
...
nrow(cardio_mort_06_10) #1465
nrow(cardio_mort_11_15) #1465
nrow(cardio_mort_16_20) #1465
```
...
data_tract <- merge(tracts10.lyr, cardio_16_20,
                    by.x="geoid10",
                    by.y="Census.Tract", 
                    all.x=TRUE)

data_tract <- data_tract %>% 
  dplyr::mutate(estimate=as.numeric(Age.Adjusted.Rate.per.100.000
)) # rename value variable for consistency
```
...
### Missing Data
The prevalence of cardiovascular disease in a community truly captures the population susceptible to environmental risk factors; however, no such publicly available data exists. Mortality data may underestimate the true population with pre-existing heart disease in the community. The Washington Department of Health, Center for Health Statistics estimates data gathered from death certificates to be 99% complete.
...
avg.value <- data_tract_nona_weight$est_weight_avg[1] #198.9801
med.value <- median(distrib) #190.68
```
*Average:* `r avg.value`
\
*Median:* `r med.value`
...
# outlier #3
data_tract20_outliers_3 <- data_tract_risk_outliers %>% 
  filter(estimate > 600)

# code to look at the scores and census tracts that are outliers
data_tract20_outliers_3
...
* Outlier #1 (53053073115): Risk score 15, Cardio mortality 434.15 - both relatively low - Elk Plain
* Outlier #2 (53033029902): Risk score 20, Cardio mortality 407.03 - both relatively low - Lakeland North/Auburn
* Outlier #3 (53033031205): Risk score 33, Cardio mortality 643.89 - low risk, high mortality rate - Lea Hill/Auburn
...
# Because the measure (cardiovascular disease mortality) does account for the population of the tract by being adjusted, it does not need to be weighted to aggregate to the region level. The map is going to visualize 2020 data, but in 2010 geographies. The original WTN data set was merged to the 2010 spatial file which has 2010 population numbers. To get 2020 population numbers, we need to aggregate population numbers in the 2020 tracts that were split from 2010 tracts. 
...
county_tract_data <- data_tract %>% 
  ...
  dplyr::mutate(cnty_estimate=mean(estimate, na.rm=TRUE)) %>% #calculate weighted value by county
  dplyr::distinct(county_name, cnty_estimate)

# calculating data by region
reg_tract_data <- data_tract %>% 
  st_drop_geometry() %>%  #to remove spatial component
  dplyr::select(estimate) %>% #to simplify table to necessary fields
  dplyr::mutate(reg_estimate=mean(estimate, na.rm=TRUE)) %>% #calculate weighted value for region
...
file_name <- 'a02-cardiovascular-disease-mortality-map-data'
...
data_equity_tracts <- merge(cardio_06_10, equity_tracts_10,
                            by.x="Census.Tract",
                            by.y="geoid", 
                            all.x=TRUE)
...
data_equity_tract10 <- data_equity_tracts
...
# join WTN data to equity quintiles (on 2010 census tracts)
data_equity_tracts <- merge(cardio_11_15, equity_tracts_15,
                            by.x="Census.Tract",
                            by.y="geoid", 
                            all.x=TRUE)
...
data_equity_tract15 <- data_equity_tracts
...
# data_crosswalk <- merge(crosswalk_10_20, cardio_16_20,
#                         by.x="geoid10",
#                         by.y="Census.Tract",
#                         all.x=TRUE)
                        
# join WTN data to equity quintiles (on 2010 census tracts)
data_equity_tracts <- merge(cardio_16_20, equity_tracts_20,
                            by.x="Census.Tract",
                            by.y="geoid", 
                            all.x=TRUE)
...
data_equity_tract20 <- data_equity_tracts 
...
data_equity_quintiles10 <- data_equity_tract10 %>% 
  ...
  dplyr::mutate(Cardio.Risk_num=as.numeric(Age.Adjusted.Rate.per.100.000))
...
```{r}
# check data sets
nrow(data_equity_pivot10) #9288
nrow(data_equity_pivot15) #9288
nrow(data_equity_pivot20) #8454
```
...
## Calculations
### Calculate by region
The cardiovascular values are available at the census tract level, but we want to calculate the regional average. We don't need to use the census bureau's tract-level population estimates (2010, 2015, 2020) to weight because the value is already adjusted by 100,000.
```{r}
data_equity_pivot10_calc <- data_equity_pivot10 %>% 
  ...
  summarise(cardio_rate=mean(Cardio.Risk_num, na.rm=TRUE)) %>% 
...
```{r}
# check data sets
nrow(data_equity_pivot10_calc) #20
nrow(data_equity_pivot15_calc) #25
...
The cardiovascular disease mortality rate values are available at the census tract level. We don't need to calculate the weighted values by the 6 equity/5 quintile groups, using the census bureau's tract-level population estimates (2010, 2015, 2020). 
```{r}
# # calculate 2010 region population
# population_test <- data_equity_pivot10 %>%
#   distinct(Census.Tract, .keep_all = TRUE) %>%
#   dplyr::summarise(pop=formatC((sum(estimate, na.rm=TRUE)), format="d", big.mark=",")) #3,603,425
# 
# sum(tract.10$estimate) #3,603,425

# calculations
data_equity_10 <- data_equity_pivot10 %>%
  ...
  dplyr::summarise(tracts=length(Census.Tract),
                   cardio_rate=mean(Cardio.Risk_num, na.rm = TRUE)) 
...
# calculations
data_equity_15 <- data_equity_pivot15 %>%
  filter(!is.na(quintile)) %>% # necessary because of added na columns from pivot
  # dplyr::mutate(tot_risk_tract=estimate*Life.Expectancy_num) %>%
  dplyr::group_by(County.Name, data_year, equity_group, quintile) %>%
  dplyr::summarise(tracts=length(Census.Tract),
...
```{r}
data_tract <- as.data.frame(rbind(data_equity_10, #equity/quintile
                                  data_equity_15,
                                  data_equity_20)) %>% 
  dplyr::select(-tracts) %>%
  rbind(data_equity_pivot10_calc, #region
...
data_clean <- data_clean %>%
  dplyr::mutate(estimate=as.numeric(round(cardio_rate),0), #rename value variable for consistency
                county=County.Name) 
...
```{r}
# set folder structure
file_name <- 'a02-cardiovascular-disease-mortality-data'
