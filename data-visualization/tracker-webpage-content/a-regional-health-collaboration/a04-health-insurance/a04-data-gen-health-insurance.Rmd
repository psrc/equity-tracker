...
indicator_measure <- 'healthcare_coverage'
indicator_title <- 'Health Insurance'
```
...
nrow(data_full) #360 in the full data set

data_full_nona <- data_full %>% 
  dplyr::filter(!is.na(fact_value))

nrow(data_full_nona) #240 - there are some NAs in the data
```
...
```{r}
num_yr_subgrp <- num_county*num_indatt #10L
```
...
#2011 is not available for health insurance
...
data_clean <- data_clean %>% 
  mutate(fact_value_notrounded=fact_value) %>% #create column to retain full share of population values
...
file_name <- 'a04-health-insurance-data'
...
county_ins <- county_acs_data %>% 
  dplyr::filter(variable=="S2701_C02_001",
                name!="Region") %>% 
  dplyr::select(name, estimate) %>% 
  dplyr::rename(ins=estimate)
county_pop <- county_acs_data %>% 
  dplyr::filter(variable=="S2701_C01_001",
                name!="Region") %>% 
  dplyr::select(name, estimate) %>% 
  dplyr::rename(pop_ins=estimate)

county_merge <- merge(county_ins, county_pop,
                      by.x="name",
                      by.y="name",
                      all.x=TRUE)
county_data_simp <- county_merge %>% 
  dplyr::mutate(cnty_estimate=ins/pop_ins) %>% 
...
reg_merge <- merge(reg_ins, reg_pop,
                      by.x="name",
                      by.y="name",
                      all.x=TRUE)
reg_data <- reg_merge %>% 
  dplyr::mutate(reg_estimate=ins/pop_ins) %>% 
...
# set folder structure
file_name <- 'a04-health-insurance-map-data'
