...
```{r}
# metric
unique(data_full$fact_type) #share of pop
...
```
```{r}
# indicator specific re-ordering and mutating
...
#                   margin_of_error = moe_sum(margin_of_error, wt_life_expec = fact_value, na.rm = TRUE))
```
...
data_clean <- data_clean %>% 
  dplyr::filter(indicator_attribute != "Less than 30 percent") %>% 
  mutate(fact_value_notrounded=round(fact_value, digits=3)) %>% #create column to retain full median income values
...
file_name <- 'e02-housing-cost-burden-data'
...
acs_data_edit <- acs_data %>% rename_at('label', ~'rent_burden')
...
acs_data_moes <- acs_data_calc %>%
  ...
  #                                        denom=estimate_new_total, 
  #                                        moe_num=moe_cost_burdened, 
  #                                        moe_denom=moe_new_total)) %>% 
...
# TEST - why moe_prop() doesn't work ----------------
# calculate moes
# acs_data_moes <- acs_data_calc %>%
#   rowwise()%>%
#   mutate(moe_new_total=tidycensus::moe_sum(estimate=c(estimate_Total, `estimate_Not computed`),
#                                            moe=c(moe_Total,`moe_Not computed`)))%>%
#   mutate(moe_perc_cost_burdened=tidycensus::moe_prop(num=estimate_cost_burdened,
#                                                      denom=estimate_new_total,
#                                                      moe_num=moe_cost_burdened,
#                                                      moe_denom=moe_new_total)) #%>%
#   # mutate(moe_cost_burdened=tidycensus::moe_sum(estimate = estimate_cost_burdened,
...
# # calculate reliability values
# acs_data_reliability <-reliability_calcs(acs_data_final,
#                                          estimate='estimate_perc_cost_burdened',
...
acs_data_tract <- acs_data_tract %>%
  ...
  dplyr::mutate(cv_no_inf = case_when(is.infinite(cv)~NA,
                                      TRUE~as.numeric(cv)))
...
psrc_palette <- leaflet::colorNumeric(palette=psrc_colors$purples_inc,
                                      domain = acs_data_tract$cv_no_inf)
...
tract_map <- leaflet() %>%
  ...
  addPolygons(data=acs_data_tract,
              fillColor = psrc_palette(acs_data_tract$cv_no_inf),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 0.7,
              group = var_name,
  ...
  addLegend_decreasing(pal = psrc_palette,
                       values = acs_data_tract$cv_no_inf,
...
# set folder structure
file_name <- 'e02-housing-cost-burden-map-data'
