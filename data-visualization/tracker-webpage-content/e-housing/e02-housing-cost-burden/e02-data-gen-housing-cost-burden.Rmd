...
data_clean <- data_clean %>% 
  ...
  mutate(fact_value=round(fact_value, digits=1)) #round median income value to hundreds - retain consistent name
```
...
acs_data <- acs_data %>% 
  dplyr::select(GEOID, rent_burden, estimate)%>%
...
acs_data_tract <- dplyr::filter(acs_data_tract,rent_burden == "Cost Burdened (More than 30 percent)")

acs_data_tract <- acs_data_tract %>% rename_at('estimate', ~'population')
acs_data_tract <- acs_data_tract %>% dplyr::mutate(estimate = (total_pop20/sum(population))*100)
```
...
# set the variable
var_name <- "Rent Burdened (Percentage)"
