...
acs_data <- acs_data %>% 
    mutate(rent_burden=factor(
             case_when(grepl("Less than 10.0 percent|10.0 to 14.9 percent|15.0 to 19.9 percent|20.0 to 24.9 percent|25.0 to 29.9 percent", rent_burden) ~
                         "Less than 30 percent",
                       grepl("30.0 to 34.9 percent|35.0 to 39.9 percent|35.0 to 39.9 percent|40.0 to 49.9 percent|50.0 percent or more", rent_burden) ~ 
...
acs_data <- acs_data %>% 
  dplyr::group_by(GEOID, rent_burden)%>%
  dplyr::summarise(estimate=sum(estimate))

# pivot data to calculate percentage by census tract
acs_data_trial <- acs_data %>% 
  pivot_wider(names_from = rent_burden, values_from = estimate) %>%
  rename_at ('Cost Burdened (More than 30 percent)', ~ 'cost_burdened')

# weight averages for population & calculating percentage/share of population per tract instead of integer
acs_data_trial$new_total <- ((acs_data_trial$Total-acs_data_trial$'Not computed'))
acs_data_trial$wt_life_expec <- ((acs_data_trial$cost_burdened/acs_data_trial$new_total)*100)

acs_data<- acs_data_trial %>% 
  dplyr::select(GEOID, cost_burdened, new_total, wt_life_expec)%>%
  dplyr::filter(!is.na(cost_burdened))
...
psrc_palette <- leaflet::colorNumeric(palette=psrc_colors$purples_inc,
                                      domain = acs_data_tract$wt_life_expec)
...
tract_map <- leaflet() %>%
  ...
  addPolygons(data=acs_data_tract,
              ...
              fillOpacity = 0.7,
              group = var_name,
              label = round(acs_data_tract$wt_life_expec, digits=1)) %>%
