...
county_reg_data_rename <- county_reg_acs_data %>% 
  mutate(rent_burden=factor(
             case_when(grepl("Less than 10.0 percent|10.0 to 14.9 percent|15.0 to 19.9 percent|20.0 to 24.9 percent|25.0 to 29.9 percent", label) ~
                         "Less than 30 percent",
                       grepl("30.0 to 34.9 percent|35.0 to 39.9 percent|35.0 to 39.9 percent|40.0 to 49.9 percent|50.0 percent or more", label) ~ 
                         "Cost Burdened (More than 30 percent)",
                       grepl("Not computed", label) ~ "Not computed",
                       !is.na(label) ~ "Total")))

county__reg_data_aggregate <- county_reg_data_rename %>% 
  dplyr::group_by(GEOID, name, rent_burden) %>% 
  dplyr::summarise(estimate=sum(estimate), .groups='drop')

# pivot data to calculate percentage by census tract
county_reg_data_group <- county__reg_data_aggregate %>% 
