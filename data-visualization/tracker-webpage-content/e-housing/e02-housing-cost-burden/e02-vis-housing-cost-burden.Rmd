...
var_name <- "Rent <br> Burdened" # the <br> could be adjusted depending on the length/spacing of the name
#labels <- sprintf(
 # "Census tract: <em>%s</em><br/><strong>%s</strong>", acs_data_tract$geoid20, # the code in <> makes the geoid value italic
  #paste("%", # if doesn't require prefix, this can be removed
   #     prettyNum((round(acs_data_tract$wt_life_expec, digits=-0)),
    #              big.mark = ","))) %>%
...
labels <- sprintf(
   ...
   paste(prettyNum((round(acs_data_tract$wt_life_expec, digits=0)), #the digits would probably be 2 for %
...
acs_map <- leaflet() %>%
  ...
  addPolygons(data=acs_data_tract,
              fillColor = psrc_palette(acs_data_tract$wt_life_expec),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 0.7,
              group = var_name,
  ...
  addLegend_decreasing(pal = psrc_palette,
                       values = acs_data_tract$wt_life_expec,
...
acs_data_tract <- acs_data_tract %>%
  rename_at ('wt_life_expec', ~ 'estimate')
...
hist(df1$x)
# regional median percentage of rent burdened population: 6.8% 
# highest rent burden households in a tract: 12%, tract 322.02; king county north of route 90 in Issaquah
...
#pums_data <- get_psrc_pums(span = 5,
 #                          dyear = 2021,
  #                         level = "h",
   #                        vars = "OWN_RENT")
#reg_median <- psrc_pums_count(pums_data, stat_var = "OWN_RENT")
...
county_acs_data <- acs_data_tract %>% 
  dplyr::arrange(desc(estimate))
```

1. 6.8%: Average percentage of cost-burdened households by census tract
2. 12%: Highest percentage of rent-burdened households in a census tract
3. 1.6%: Lowest proportion of rent-burdened households

\

#### Insights & Analysis

* King County has the highest population in the region and the highest percentage of rent-burdened households (12%)
* The five census tracts tied with the highest representation of cost-burdened households are in the Issaquah Highlands (12%) and Hollywood (10.7%) in King County, Clearview (11%) in Snohomish County, and the Fort Lewis (10.7%) and West Frederickson (10.5%) in Pierce County
* The three census tracts with the lowest representation of rent-burdened households are Seattle's Bellevue (1.6%), Snohomish County's southern tip of Edmonds (1.7%), and Seattle's Georgetown neighborhood (2%) 
...
data_clean_column <- data_clean %>% 
  ...
  dplyr::filter(indicator_attribute != "Less than 30 percent") #filter on most recent year
...
1. 2.7x: Households who are living below 200% of the poverty level are 2.7 times more likely to be cost-burdened than households who are living above it
2. 60%: The percentage of Non-English proficient households that are at risk of being rent burdened across the region
3. 50%: Snohomish County narrows the gap between people with disabilities and those living without, as well as households with older adults (65+ years old) nd those without

\

#### Insights & Analysis

* Households that are living below 200% of the poverty level are the households at highest risk of experiencing rent burden, and are equally represented across all counties.
* King County has the highest gap between households with English proficiency and households that are non-English proficient (20%), versus other counties with smaller gaps (10%)
* Snohomish County narrows the gap between households with people with disabilities and households with older adults (65+ years old) from 20% to 10%, and narrows the gap for households with people of color to 0%
...
1. 1.5%: In 2021, the percentage points between households in the region who were living below 200% of the poverty line and households living above
2. 50%: Regionally, people of color are as likely to be rent burdened as white non-Hispanic households, a proportion which has remained relatively consistent in the last ten years
3. 0%: Percentage increase for households with and without people living with disabilities

\

#### Insights & Analysis

* In 2021, the difference between households in the region who were below 200% of the poverty line and households above the 200% poverty line was 1.5%, which has remained consistent, but the proportion of rent burdened households based on income has increased by 166% from 2011
* Since 2011, King County is the only county where the gap between households with youth (18 years or younger) has grown (from 50% to 60%), whereas households without youth decreased their rent burden (from 50% to 40%)
* Households with older adults that are rent-burdened have increased in King and Snohomish County, while they have decreased the gap between households in Kitsap and Pierce counties.
