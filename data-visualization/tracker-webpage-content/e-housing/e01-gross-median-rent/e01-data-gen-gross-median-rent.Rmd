...
indicator_title <- 'Gross Median Rent'
```
...
# clean up to median rent per census tract
acs_data <- acs_data %>% rename_at ('variable', ~ 'column')

acs_data <- dplyr::select(filter(acs_data, column == 'B25031_001'), c('GEOID', 'column', 'label', 'estimate', 'moe')) %>%
...
acs_data <- acs_data %>% 
  dplyr::select(GEOID, estimate, moe, reliability)%>%
...
# set folder structure
file_name <- 'e01-gross-median-rent-map-data'
