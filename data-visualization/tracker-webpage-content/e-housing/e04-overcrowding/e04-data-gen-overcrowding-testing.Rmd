...
indicator_title <- 'Renter Overcrowding (>1 person/room)'
```
...
data_clean <- data_clean %>% 
  filter(indicator_attribute=="More than 1.5 persons per room") %>% 
...
acs_data_group <- acs_data_filter %>% 
  ...
  dplyr::summarise(estimate=sum(estimate), 
                   moe=tidycensus::moe_sum(estimate=estimate, moe=moe)) %>% 
  dplyr::filter(per_room != "Less than or equal to 1 person per bedroom")
...
totals <- dt_21 %>% 
  ...
  dplyr::filter(rent_crowding=="More than 1 person per room") %>% 
