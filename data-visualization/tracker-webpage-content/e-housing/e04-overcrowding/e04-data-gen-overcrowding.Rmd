...
acs_data_trial <- acs_data %>% rename_at('label', ~'per_bedroom')%>%
...
acs_data_trial <- acs_data_trial %>%
  dplyr::filter(column %in% row_names) %>%
  mutate(per_bedroom = factor(
    case_when(per_bedroom == "Estimate!!Total:!!Owner occupied:" ~ "Owner_occupied_total",
              per_bedroom == "Estimate!!Total:!!Renter occupied:" ~ "Renter_occupied_total",
                                   TRUE ~ as.character(per_bedroom)))) %>%
  mutate(per_bedroom=factor(
             case_when(grepl("0.50 or less|0.51 to 1.00|1.01 to 1.50", per_bedroom) ~ "Less than 1.5 persons per bedroom",
                       grepl("1.51 to 2.00|2.01 or more", per_bedroom) ~ "Overcrowding (More than 1.5 persons per bedroom)",
                       TRUE ~ as.character(per_bedroom))))
...
# set the variable
var_name <- "Overcrowding (>1.5 persons)"
