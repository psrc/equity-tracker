...
```{r acs data - county level, include=FALSE, eval=FALSE}
# Finding the corresponding ACS table - this works if you know the correct concept label. If not, another option would be to visit https://data.census.gov/table and search for the right subject table and skip to the next step ----
...
```{r pums data - county level}
# access pums data - 2021
crowd_2021 <- get_psrc_pums(span = 5,
                            dyear = 2021,
                            level = "h", # Unit of analysis == household
                            vars = c("NP","BDSP", "OWN_RENT")) # Number of persons in household, Number of bedrooms, Own/rent tenure status

# calculate NP/BDSP
crowding <- crowd_2021 %>% 
  dplyr::mutate(crowding = factor(case_when(is.na(NP)|is.na(BDSP)|BDSP==0 ~ NA_character_, 
                ...
                rent_crowding = factor(case_when(is.na(NP)|is.na(BDSP)|BDSP==0 ~ NA_character_, 
...
totals <- dt_21 %>% 
  ...
  dplyr::filter(rent_crowding=="More than 1.5 persons per bedroom") %>% 
...
# save final data set as .rda
save(pums_data_county, file = file.path(base_dir,
                                      theme_dir,
                                      ind_dir, "rda-data",
                                      paste0(file_name,'.rda')))
```
