...
indicator_measure <- 'renter_crowding'
indicator_title <- 'Renter Overcrowding (>1 person/bedroom)'
```
...
unique(data_full$indicator_attribute) #3
...
```{r}
num_yr_subgrp <- num_county*num_indatt #15
```
...
data_clean$indicator_attribute <- factor(data_clean$indicator_attribute,
                                    levels = c("One person per room or less",
                                               "More than 1 person per room"))

# data_clean2 <- data_clean %>% 
#   filter(indicator_attribute=="Between 1 and 1.5 person(s) per room" |
#            indicator_attribute=="More than 1.5 persons per room")
```
\
...
facet_chart <- static_facet_column_chart(t=just_region_21,
                                         x="indicator_attribute", y="fact_value",
                                         fill="vulnerability", facet="focus_type_ord",
                                         est = "percent",
                                         scales= "fixed",
                                         color = "psrc_pairs", 
...
data_clean <- data_clean %>% 
  filter(indicator_attribute=="More than 1 person per room") %>% 
  mutate(fact_value_notrounded=fact_value) %>% #create column to retain full median income values
  mutate(fact_value=round(fact_value, digits=2)) #round percentage to whole percentage - retain consistent name
```
...
county.lyr <- st_read_elmergeo('wa_counties_water_removed')
places.lyr <- st_read_elmergeo('place2020')
...
```{r acs data - tract level, include=FALSE, eval=FALSE}
...
acs_data <- get_acs_recs(geography ='tract', 
                         table.names = 'B25014', #room
                         # table.names = 'B25042', #bedroom
                         years = c(2021),
                         acs.type = 'acs5')
...
acs_data_filter <- acs_data %>%
  ...
  mutate(per_bedroom = factor(
    case_when(variable == "B25014_002" ~ "Total",
              ...
              grepl("0.50 or less|0.51 to 1.00", label) ~ "Less than or equal to 1 person per bedroom",
              grepl("1.01 to 1.50|1.51 to 2.00|2.01 or more", label) ~ "Overcrowding",
...
acs_data_group <- acs_data_filter %>% 
  ...
  dplyr::filter(per_bedroom != "Less than or equal to 1 person per bedroom")
...
```{r acs data - place level, include=FALSE, eval=FALSE}
# Finding the corresponding ACS table - this works if you know the correct concept label. If not, another option would be to visit https://data.census.gov/table and search for the right subject table and skip to the next step ----
...
# use the moe_sum function from tidycensus
acs_data_group <- acs_data_filter %>% 
  dplyr::group_by(GEOID, per_room, type) %>% #add 'type' if want to distinguish b/t Owners and Renters
  dplyr::summarise(estimate=sum(estimate), 
                   moe=tidycensus::moe_sum(estimate=estimate, moe=moe)) %>% 
  dplyr::filter(per_room != "Less than or equal to 1.5 persons per bedroom")

# pivot data to calculate percentage by census tract
acs_data_pivot<- acs_data_group %>%   
  pivot_wider(names_from = c(per_room, type), values_from = c(estimate,moe)) #add 'type' if want to distinguish b/t Owners and Renters
...
# connect to place shapefile to get names instead of just geoid
acs_data_place <- merge(places.lyr, acs_data,
                        by.x="geoid20",
                        by.y="GEOID",
                        all.x=FALSE)
...
# library("writexl")
# write_xlsx(acs_data_place,"T:/2023November/Mary/EquityTracker/temp/overcrowding_renters_acs_place.xlsx")
...
# use the moe_sum function from tidycensus
acs_data_group <- acs_data_filter %>% 
  dplyr::group_by(GEOID, name, per_bedroom, type) %>% #add 'type' if want to distinguish b/t Owners and Renters
  dplyr::summarise(estimate=sum(estimate), 
                   moe=tidycensus::moe_sum(estimate=estimate, moe=moe)) %>% 
  dplyr::filter(per_bedroom != "Less than or equal to 1 person per bedroom")
...
acs_data <- acs_data_reliability_rent %>% #just interested in renters for now
  rename(estimate_dec='estimate_perc_overcrowding_rent') %>%
  mutate(estimate=estimate_dec*100) %>% 
  dplyr::select(GEOID,county_name,estimate,reliability)

acs_data_cnty <- acs_data %>% 
  dplyr::filter(county_name!="Region")

acs_data_reg <- acs_data %>% 
  dplyr::filter(county_name=="Region")

# add region median to county data
county_reg_data <- acs_data_cnty %>%
  mutate(reg_estimate=acs_data_reg$estimate)
...
```{r pums data - county level, include=FALSE, eval=FALSE}
# access pums data - 2021
crowd_2021 <- get_psrc_pums(span = 5,
                            dyear = 2021,
                            level = "h", # Unit of analysis == household
                            vars = c("NP", # Number of persons in household
                                     "RMSP", # Number of rooms
                                     # "BDSP", # Number of bedrooms
                                     "OWN_RENT")) # Own/rent tenure status

# calculate NP/BDSP
crowding <- crowd_2021 %>% 
  dplyr::mutate(crowding = factor(case_when(is.na(NP)|is.na(RMSP)|RMSP==0 ~ NA_character_, 
                                            NP/RMSP <= 1 ~ "One person per room or less",
                                            NP/RMSP <= 1.5 ~ "Between 1 and 1.5 person(s) per room",
                                            NP/RMSP  > 1.5 ~ "More than 1.5 persons per room"),
                                  levels=c("More than 1.5 persons per bedroom",
                                           "Between 1 and 1.5 person(s) per bedroom",
                                           "One person per bedroom or less")),
                rent_crowding = factor(case_when(is.na(NP)|is.na(RMSP)|RMSP==0 ~ NA_character_, 
...
# for all households
# dt_21 <- psrc_pums_count(crowding, 
#                          group_vars=c("crowding","COUNTY"), 
#                          rr = TRUE)
# for renter households
dt_21 <- psrc_pums_count(crowding, 
                         group_vars=c("rent_crowding","COUNTY"), 
                         rr = TRUE)

totals <- dt_21 %>% 
  dplyr::filter(!is.na(rent_crowding),
                rent_crowding != "Total") %>% 
  dplyr::group_by(COUNTY) %>% 
  dplyr::mutate(geo_total_rent=sum(count),
                share_rent=count/geo_total_rent) %>% 
  dplyr::filter(rent_crowding=="More than 1.5 persons per room") %>% 
  dplyr::select(-reliability, - share, -share_moe)


# calculate moes/relative reliability -----
totals_moes <- totals %>%
  rowwise() %>% 
  dplyr::mutate(moe_rent=tidycensus::moe_sum(estimate = count,
                                             moe = count_moe))
                # moe_own=tidycensus:moe_prop(num=estimate_Overcrowding_Owner,
                #                           denom=estimate_Total_units,
                #                           moe_num=moe_Overcrowding_Owner,
                ...
                # moe_own=tidycensus::moe_sum(estimate = estimate_Overcrowding_Owner,
                #                             moe = moe_Overcrowding_Owner))
totals_final<- totals_moes %>% 
  dplyr::filter(!is.na(count))

# calculate reliability values
totals_reliability_rent <- reliability_calcs(totals_final,
                                             estimate='count',
                                             moe='moe_rent')
# acs_data_reliability_own <- reliability_calcs(acs_data_final,
#                                               estimate='estimate_Overcrowding_Owner',
...
# filter county data
county_data <- totals_reliability_rent %>% 
  dplyr::filter(COUNTY != "Region") %>% 
  dplyr::rename(cnty_estimate=share_rent)

# filter region data
region_data <- totals_reliability_rent %>%
  dplyr::filter(COUNTY == "Region") %>%
  dplyr::rename(reg_estimate=share_rent) %>%
  dplyr::select(COUNTY, reg_estimate) #simplify table
...
# rename and simplify
pums_data <- county_reg_data %>%
  rename(estimate_dec='cnty_estimate') %>%
  mutate(estimate=estimate_dec*100) %>% 
  rename(reg_estimate_dec='reg_estimate') %>% 
  mutate(reg_estimate=reg_estimate_dec*100) %>% 
  dplyr::select(COUNTY,estimate,reg_estimate, reliability)
...
#   rename(estimate_dec='estimate_perc_overcrowding_rent') %>%
#   mutate(estimate=estimate_dec*100) %>% 
#   dplyr::select(PUMA5CE,estimate,cv,reliability)
```

```{r acs data - puma level, include=FALSE, eval=FALSE}
# Finding the corresponding ACS table - this works if you know the correct concept label. If not, another option would be to visit https://data.census.gov/table and search for the right subject table and skip to the next step ----
...
# import tracts to pumas crosswalk
tracts_pumas_crosswalk <- read.csv("T:/2023November/Mary/EquityTracker/crosswalk/Census_Tract_to_2020_PUMA.txt")

psrc_pumas_crosswalk <- tracts_pumas_crosswalk %>% 
  dplyr::filter(STATEFP==53 &
                  (COUNTYFP==33 | 
                     COUNTYFP==35 |
                     COUNTYFP==53 |
                     COUNTYFP==61)) %>% 
  mutate(tract_6=as.character(sprintf("%06d",TRACTCE)))

# create field in acs data for merge (last 6 digits to match puma data)
acs_for_merge <- acs_data_filter %>% 
  dplyr::mutate(tract_6=str_sub(GEOID, start= -6))

# merge pumas crosswalk to acs data
merged_tract_puma <- merge(acs_for_merge, psrc_pumas_crosswalk,
                           by.x="tract_6",
                           by.y="tract_6",
                           all.x=TRUE)

# # testing to make sure numbers line up
# one_puma <- merged_tract_puma %>% 
#   dplyr::filter(PUMA5CE==23318,
#                 per_bedroom=="Overcrowding")
# 
# one_puma_own <- one_puma %>% 
#   dplyr::select(-tract_6, -name, -state, -variable, 
#                 -concept, -acs_type, -year, -census_geography, -STATEFP, -COUNTYFP) %>% 
#   dplyr::filter(type=="Owner") %>% 
#   dplyr::mutate(sq=(moe^2),
#                 sum_sq=sum(sq),
#                 sq_rt=sqrt(sum_sq),
#                 num1=sq_rt/1.645,
#                 sum_est=sum(estimate),
#                 cv_final=num1/sum_est)

# ----------------
# for separate owners and renters
# use the moe_sum function from tidycensus
acs_data_group <- merged_tract_puma %>% 
  dplyr::group_by(PUMA5CE, per_bedroom, type) %>% #add 'type' if want to distinguish b/t Owners and Renters
  dplyr::summarise(estimate=sum(estimate), 
                   moe=tidycensus::moe_sum(estimate=estimate, moe=moe)) %>% 
  dplyr::filter(per_bedroom != "Less than or equal to 1.5 persons per bedroom")
...
# use the moe_sum function from tidycensus
acs_data_group <- merged_tract_puma %>% 
  dplyr::group_by(PUMA5CE, per_bedroom) %>% #add 'type' if want to distinguish b/t Owners and Renters
  dplyr::summarise(estimate=sum(estimate), 
                   moe=tidycensus::moe_sum(estimate=estimate, moe=moe)) %>% 
  dplyr::filter(per_bedroom != "Less than or equal to 1.5 persons per bedroom")
...
# acs_data_merge <- merge(tracts20.lyr, acs_data,
#                         by.x="geoid20",
#                         by.y="GEOID",
#                         all.x=TRUE)

# merge to 2020 census tract spatial file
acs_data <- merge(county.lyr, county_reg_data,
                  by.x="county_name",
                  by.y="county_name",
                  all.y=TRUE)

# # merge to 2020 county spatial file
# pums_data_county <- merge(county.lyr, pums_data,
#                           by.x="county_name",
#                           by.y="COUNTY",
#                           all.x=FALSE)
...
# add region to county data
# region_data <- acs_data %>% 
#   dplyr::filter(county_name=="Region") %>% 
#   dplyr::mutate(reg_estimate=estimate)


# # merge tract, county, and region data into one data set 
# acs_data_tract <- merge(acs_data_merge, county_reg_data,
#                         by.x="county_name",
#                         by.y="county_name",
#                         all.x=TRUE)

# merge county and region data into one data set 
# acs_data_tract <- cbind(acs_data_merge, region_data$reg_estimate) %>% 
#   dplyr::rename(reg_estimate=region_data.reg_estimate)
```
...
# # load puma shapefile
# puma20.url <- "https://services6.arcgis.com/GWxg6t7KXELn1thE/arcgis/rest/services/Census_PUMAS_2020/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson"
# 
# puma20.lyr<-st_read(puma20.url)
# 
# nrow(puma20.lyr) #34
# 
# # merge puma shapefile to puma level data
# acs_data_tract_merge <- merge(puma20.lyr, acs_data,
#                               by.x="pumace20",
#                               by.y="PUMA5CE",
#                               all.x=TRUE)
# 
# reliability_order <- c("good",
#                        "fair",
#                        "use with caution",
#                        "use with extreme caution")
# 
# acs_data_tract <- acs_data_tract_merge %>% 
#   dplyr::mutate(reliability_ord = factor(reliability, levels = reliability_order))
...
psrc_palette <- leaflet::colorNumeric(palette=psrc_colors$purples_inc,
                                      domain = acs_data_place$estimate)
# psrc_palette <- leaflet::colorFactor(palette=psrc_colors$purples_inc,
#                                       domain = acs_data_tract$reliability_ord)
...
tract_map <- leaflet() %>%
  ...
  leaflet::addProviderTiles("CartoDB.VoyagerOnlyLabels",
                            ...
                            group = "Labels") %>%
  addPolygons(data=acs_data_place,
              fillColor = psrc_palette(acs_data_place$estimate),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 0.7,
              group = var_name,
              label = round(acs_data_place$estimate, digits=1)) %>%

  # legends
  addLegend_decreasing(pal = psrc_palette,
                       values = acs_data_place$estimate,
...
```{r explore map, include=FALSE, eval=FALSE}
# set map extent
map.lat<- 47.615
map.lon<- -122.257
map.zoom<- 8.5
...
# save final data set as .rda
save(acs_data, file = file.path(base_dir,
                                      theme_dir,
                                      ind_dir, "rda-data",
                                      paste0(file_name,'.rda')))
```
