...
acs_data <- base_acs_data %>% rename_at('label', ~'tenure')%>%
  rename_at ('variable', ~ 'column')

acs_data <- dplyr::select(filter(acs_data, column== 'S2502_C03_001'| column== 'S2502_C01_001'), c('GEOID', 'estimate', 'tenure')) %>%
  mutate(tenure = factor(case_when(tenure == "Estimate!!Owner-occupied housing units!!Occupied housing units" ~ "Owner_occupied_units",
                                   TRUE ~ 'Total_units')))
...
acs_data$estimate <- ((acs_data$Owner_occupied_units/acs_data$Total_units)*100)
...
acs_data <- acs_data %>% 
  ...
  dplyr::filter(!is.na(estimate)) #estimate = total owner-occupied housing units in percentage
