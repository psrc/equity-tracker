...
```
<span style="color: #00A7A0">
The disability and LEP category is missing all data in 2011.
</span> 
\
\
...
file_name <- 'e03-homeownership-rates-data'
...
#x <- tidycensus::load_variables(2021,"acs5") %>% 
 # dplyr::filter(grepl("homeownership",
  #                    concept, ignore.case=TRUE) & 
   #               (geography=="tract"))

# getting data by tract
base_acs_data <- get_acs_recs(geography ='tract', 
                              table.names = 'S2502', #subject table code
                              years = c(2021),
                              acs.type = 'acs5')

# filter to the variables of interest
acs_data_filter <- base_acs_data %>% 
  dplyr::filter(variable == 'S2502_C03_001'| 
                  variable== 'S2502_C01_001') %>%
  dplyr::mutate(tenure = factor(case_when(label == "Estimate!!Owner-occupied housing units!!Occupied housing units" ~ "Owner_occupied_units",
                                          TRUE ~ 'Total_units'))) %>% 
  dplyr::select(GEOID, variable, tenure, estimate, moe)

# test <- acs_data_filter %>%
#   dplyr::select(GEOID, estimate, moe, label, reliability, tenure)

# use the moe_sum function from tidycensus
acs_data_group <- acs_data_filter %>% 
  dplyr::group_by(GEOID, tenure)%>%
...
acs_data_pivot<- acs_data_group %>%   
  pivot_wider(names_from = tenure, values_from = c(estimate,moe))

# calculating percentage/share of population per tract instead of integer
acs_data_calc <- acs_data_pivot %>%
  mutate(estimate_perc_own_occ=estimate_Owner_occupied_units/estimate_Total_units)

# calculate moes
acs_data_moes <- acs_data_calc %>%
  rowwise() %>%
  # mutate(moe_perc_own_occ=tidycensus::moe_prop(num=estimate_Owner_occupied_units,
  #                                              denom=estimate_Total_units,
  #                                              moe_num=moe_Owner_occupied_units,
  #                                              moe_denom=moe_Total_units)) %>%
  mutate(moe_perc_own_occ=tidycensus::moe_sum(estimate = estimate_Owner_occupied_units,
                                               moe = moe_Owner_occupied_units))
...
county_reg_data_aggregate <- county_reg_data_rename %>% 
  dplyr::group_by(GEOID, name, tenure) %>% 
...
county_reg_data_group <- county_reg_data_aggregate %>% 
  ...
  dplyr::mutate(own_occ_perc = ((Owner_occupied_units/Total_units)*100),
...
county_data <- county_reg_data_group %>% 
  ...
  dplyr::rename(cnty_estimate=own_occ_perc) %>% #to distinguish between tract and county values
  dplyr::select(county_name, cnty_estimate) #simplify table

# getting data by region
region_data <- county_reg_data_group %>% 
  dplyr::filter(name == "Region") %>% 
  dplyr::rename(reg_estimate=own_occ_perc) %>% #to distinguish between tract and county values
...
# set folder structure
file_name <- 'e03-homeownership-rates-map-data'
