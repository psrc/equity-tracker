...
x <- tidycensus::load_variables(2021,"acs5") %>% 
  dplyr::filter(grepl("housing cost burden",
...
acs_data <- acs_data$label %>% 
  dplyr::mutate(label = factor(label,
  case_when(grepl("^(Less than 10.0 percent|10.0 to 14.9 percent|15.0 to 19.9 percent|20.0 t 24.9 percent|
                    25.0 to 29.9 percent)" ~ "Less than 30 percent",
                    grepl("^(30.0 to 34.9 percent|35.0 to 39.9 percent|35.0 to 39.9 percent|40.0 to 49.9 percent|
                    50.0 percent or more)" ~ "Cost Burdened (More than or Equal to 30 percent)"))))) %>%
  filter(label != "Total")
  ...
  mutate(
  ESR= factor(
    case_when(grepl("^(Armed forces, at work|Civilian employed, at work|Armed forces, with a job but not at work|
                    Civilian employed, with a job but not at work)" ~ "Employed",
              grepl("^(Not in labor force|Unemployed" ~ "Unemployed"))),
