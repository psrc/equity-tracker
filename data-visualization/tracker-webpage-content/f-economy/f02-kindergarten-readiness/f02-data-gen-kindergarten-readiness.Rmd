...
# Download data 
## Primary data 
The data set referenced in this script is generated from the American Community Survey (ACS) and the Washington Office of Superintendent of Public Instruction [(OSPI)](https://www.k12.wa.us/data-reporting/data-portal). These data sets provide data at the **person- or household-level**, with the ability to look at the different indicators by the six equity demographic groups of interest. 
```{r, include=FALSE}
# PUMS/OSPI data set
...
# equity focus groups
num_group <- length(unique(data_full$focus_type)) #4 - age groups not included for this data set
...
```{r}
# metric
unique(data_full$fact_type) #fraction 6 for 6
...
unique(data_full$indicator_attribute) #6 for 6 dimensions
...
There is missing data in Kitsap and Snohomish in the earlier years.
</span> 
\
\
...
```
<span style="color: #00A7A0">
All of the categories are missing data in the earlier years (up throguh 2014).
</span> 
\
\
...
focus_type_order <- c("Students of Color", 
                      "Students in Households with Lower Income", 
...
data_clean <- data_full %>% 
  ...
  mutate(focus_type_ord = case_when(
    focus_type=="POC_cat"~"Students of Color",
    focus_type=="Disability_cat"~"Students with a Disability",
    focus_type=="LEP_cat"~"Students with Limited English Proficiency",
    focus_type=="Income_cat"~"Students in Households with Lower Income",
...
data_clean <- data_clean %>% 
  mutate(fact_value_notrounded=fact_value) %>% #create column to retain full values
  mutate(fact_value=round(fact_value, digits=2)) %>% #round percentage value to hundredths - retain consistent name
  filter(fact_value>0) #removing the one value that was 0 (2022 in Kitsap)
```
...
file_name <- 'f02-kindergarten-readiness-data'
...
sch_districts <- st_read_elmergeo('school_districts_no_water')
```
...
kgreadiness_schdistrict <- read.csv(file.path(base_dir,
                                              theme_dir,
                                              ind_dir, "raw-data",
...
# set folder structure
file_name <- 'f02-kindergarten-readiness-map-data'

# save final data set as .rda
save(ospi_data_tract, file = file.path(base_dir,
                                       theme_dir,
                                       ind_dir, "rda-data",
                                       paste0(file_name,'.rda')))
```
