...
library(magrittr)
library(ggplot2)
library(tidyr) #moe calculations
...
indicator_measure <- 'educational_attainment'
indicator_title <- 'Educational attainment'
```
...
unique(data_full$data_year) #2012, 2017, 2022

# indicator attributes
num_indatt <- length(unique(data_full$indicator_attribute)) #2
unique(data_full$indicator_attribute) #"Bachelor's degree or higher"   "Less than a Bachelor's degree"
...
# filter to "Bachelor's degree or higher"
data_clean <- data_clean %>% 
  filter(indicator_attribute=="Bachelor's degree or higher")
```
\
...
data_clean <- data_clean %>% 
  ...
  mutate(fact_value=round(fact_value, digits=2)) #round educational attainment value to hundredths - retain consistent name

data_clean #check to make sure the rounding worked - if not will need to go back to generate data set in chunk around line 245
```
...
file_name <- 'f03-educational-attainment-data'
...
#   dplyr::filter(grepl("education attainment",
...
# filter only fields of interest - census tract and population (may need to be adjusted depending on indicator and if you need to run any calculations/transformations) - look at labels and filter on variable if necessary
...
bach_plus_data <- base_acs_data %>% 
  dplyr::filter(variable=="S1501_C01_015") %>% 
  dplyr::select(GEOID, estimate, moe) %>% 
  dplyr::rename(bach_plus_estimate=estimate,
                bach_plus_moe=moe)

pop_25_plus_data <- base_acs_data %>% 
  dplyr::filter(variable=="S1501_C01_006") %>% 
  dplyr::select(GEOID, estimate, moe) %>% 
  dplyr::rename(pop_25_estimate=estimate,
                pop_25_moe=moe)

merge_data <- merge(bach_plus_data, pop_25_plus_data,
                    by.x="GEOID",
                    by.y="GEOID",
                    all.x=TRUE)

# #just for 2017
# bach_plus_data_12 <- base_acs_data %>% 
#   dplyr::filter(variable=="S1501_C01_012") %>% 
#   dplyr::select(GEOID, estimate) 
# 
# bach_plus_data_13 <- base_acs_data %>% 
#   dplyr::filter(variable=="S1501_C01_013") %>% 
#   dplyr::select(GEOID, estimate) 
# 
# bach_plus_data <- left_join(bach_plus_data_13, bach_plus_data_12, by = "GEOID")
# bach_plus_data <- bach_plus_data %>%
#   pivot_longer(!GEOID, names_to = "degree", values_to = "est")
# 
# bach_plus_data <- bach_plus_data %>%
#     group_by(GEOID) %>% 
#   summarise(bach_plus_estimate = sum(est))
# 
# write.csv(merge_data, "merge.csv")
...
# acs_data_pivot <- acs_data %>% 
#   pivot_wider(names_from = label, values_from = estimate) %>% 
#   dplyr::rename(bach_plus="Estimate!!Total!!AGE BY EDUCATIONAL ATTAINMENT!!Population 25 years and over!!Bachelor's degree or higher",
#                 pop_25_plus="Estimate!!Total!!AGE BY EDUCATIONAL ATTAINMENT!!Population 25 years and over") %>% 
#   mutate(estimate=bach_plus/pop_25_plus) #consistent variable name

# calculate moes
acs_data_group <- merge_data %>%
  rowwise() %>%
  mutate(estimate=bach_plus_estimate/pop_25_estimate) %>% 
  mutate(moe_perc_edu=tidycensus::moe_prop(num=bach_plus_estimate, denom=pop_25_estimate, 
                                           moe_num=bach_plus_moe, moe_denom=pop_25_moe))

acs_data_final<-reliability_calcs(acs_data_group, 
                                  estimate='estimate', 
                                  moe='moe_perc_edu')
```
...
acs_data_tract_merge <- merge(tracts20.lyr, acs_data_final,
                              by.x="geoid20",
                              by.y="GEOID",
                              all.x=TRUE)
...
county_bach <- county_acs_data %>% 
  dplyr::filter(variable=="S1501_C01_015",
                name!="Region") %>% 
  dplyr::select(name, estimate) %>% 
  dplyr::rename(bach_plus=estimate)
county_pop <- county_acs_data %>% 
  dplyr::filter(variable=="S1501_C01_006",
                name!="Region") %>% 
  dplyr::select(name, estimate) %>% 
  dplyr::rename(pop_plus=estimate)

county_merge <- merge(county_bach, county_pop,
                      by.x="name",
                      by.y="name",
                      all.x=TRUE)
county_data_simp <- county_merge %>% 
  dplyr::mutate(cnty_estimate=bach_plus/pop_plus) %>% 
...
reg_merge <- merge(reg_bach, reg_pop,
                      by.x="name",
                      by.y="name",
                      all.x=TRUE)
reg_data <- reg_merge %>% 
  dplyr::mutate(reg_estimate=bach_plus/pop_plus) %>% 
...
tract_map <- leaflet() %>%
  ...
  addPolygons(data=acs_data_tract,
              ...
              fillOpacity = 0.7,
              group = var_name,
              label = round(acs_data_tract$estimate, digits=2)) %>%
...
# set folder structure
file_name <- 'f03-educational-attainment-map-data'
