...
data <- 1:4 #number of datasets in raw_data list - to iterate through
```
...
# Download data 
## Primary data 
These data come from the [Washington Secretary of State webpage](https://www.sos.wa.gov/elections#data_research). Separate xlxs and csv files are available for each election year with voting data by precinct. The data are available back to 2007, but we will be focusing on presidential election years, every 4 years, and going as far back as 2012 (considering 2010 and 2020 census geographies). More information about the pre-processing of these data are available in GitHub (GitHub/equity-tracker/data_development) in 1-voter-turnout-exploration.qmd, 2-voter-turnout-ofm-acs-dec.qmd, and 3-voter-turnout-12-16-20-24.qmd. The transformation between precinct-tract is done in 3-voter-turnout-12-16-20-24.qmd.
```{r}
df_raw_data <- read.csv('Y:/Equity Indicators/tracker-webpage-content/h-public-services/h02-voter-participation/raw-data/voter_participation.csv')
```
...
# check data
str(df_raw_data)
# table(df_raw_data$election_year)
```
...
df_psrc_data <- df_raw_data %>% 
  select(-X) %>% #remove id column from CSV
  mutate(data_year = election_year)
...
tract_indicator <- df_psrc_data %>% # insert the name of your data set
  rename(variable_value=pop18_turnout, # insert the name of your indicator variable
...
num_outliers <- nrow(data_tract_outliers) #6
```
\
...
nrow(data_tract_risk_outliers) #6
pop_outliers <- as.numeric(sum(data_tract_risk_outliers$total_pop20, na.rm = T)) #33,324
# There are approx. 33,324 people (2020 population) in these outlier tracts - just for a broad estimate.
regvot_outliers <- as.numeric(sum(data_tract_risk_outliers$reg_voters, na.rm = T)) #4,482
# There are approx. 4,482 registered voters in these outlier tracts.
```
...
county_tract_data <- data_tract %>%
  st_drop_geometry() %>%  #to remove spatial component
  dplyr::group_by(County_Name) %>%
  dplyr::mutate(cnty_estimate=(sum(ballots_cast, na.rm=TRUE))/(sum((total_pop_ofm*pop18_total), na.rm=TRUE))) %>% #calculate average value by county
...
reg_tract_data <- data_tract %>%
  st_drop_geometry() %>%  #to remove spatial component
  dplyr::mutate(reg_estimate=(sum(ballots_cast, na.rm=TRUE))/(sum(total_pop_ofm*pop18_total, na.rm=TRUE))) %>% #calculate average value for region
  dplyr::distinct(reg_estimate)
...
# need to join 2024 election data to 2023 equity quintiles -----
equity_tracts_adj <- equity_tracts %>% 
  mutate(data_year=case_when(data_year==2023 ~ 2024,
                             TRUE~data_year))

data_equity_tracts <- merge(tract_indicator, equity_tracts_adj,
                            by=c("geoid", "data_year"),
                            all.x=TRUE)

# check data set
nrow(data_equity_tracts) #6764
...
data_tract <- data_equity_pivot %>%
  ...
  summarise(tracts=length(geoid),
            ...
            total_pop=(sum((total_pop_ofm*pop18_total), na.rm=TRUE)),
            .groups = "drop") %>%
...
low_lowmed_years <- unique(low_lowmed$data_year) #2016 and 2020
...
data_tract_comb <- rbind(low_lowmed, #6
                         other_data) #568

nrow(data_tract_comb) #574
