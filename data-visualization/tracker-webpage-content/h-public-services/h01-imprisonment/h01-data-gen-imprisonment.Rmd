...
avg.value <- data_tract_nona$estimate[1] #5.60 - pulling first value
med.value <- median(distrib) #4.6
```
*Average:* `r avg.value`
\
*Median:* `r med.value`
...
data_equity_quintiles20 <- data_equity_tract20 %>% 
  ...
  dplyr::mutate(Log_rate_num=as.numeric(Log_rate))
...
data_equity_pivot20_calc <- data_equity_pivot20 %>% 
  ...
  summarise(wt_Log_rate=(sum(Log_rate_num*estimate, na.rm =TRUE))/(sum(estimate, na.rm=TRUE))) %>%
...
data_equity_wt20 <- data_equity_pivot20 %>%
  ...
  dplyr::mutate(tot_risk_tract=estimate*Log_rate_num) %>%
  ...
  dplyr::mutate(wt_Log_rate=tot_rate/tot_pop) 
...
data_clean <- data_clean %>%
  dplyr::mutate(estimate=as.numeric(wt_Log_rate)) #rename value variable for consistency
