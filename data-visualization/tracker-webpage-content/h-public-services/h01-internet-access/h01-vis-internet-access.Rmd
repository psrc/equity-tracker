...
rda <- 'h01-internet-access-map-data.rda'
...
# psrc_palette <- leaflet::colorNumeric(palette = psrc_purple_plus,
#                                       domain = c(min(acs_data_tract$estimate,na.rm = TRUE),max_value, NA)) # this is the value field

# setting the legend/label variables
var_name <- "Internet <br> Access" # the <br> could be adjusted depending on the length/spacing of the name
labels <- sprintf(
...
acs_map
```
Source(s): U.S. Census Bureau, 2017-2021 American Community Survey 5-Year Estimates, Table S2801; U.S. Census Bureau, Geography Division 2020 TIGER/Line Shapefiles
```{r generate acs map html, include=FALSE}
# to save html visual to folder, you must create an 'webpage-html-outputs' folder within each indicator sub-folder: Y:\Equity Indicators\tracker-webpage-acs_mapcontent\f-economy\f01-median-income\webpage-html-outputs
...
# 53033005303 (UW/U-District, 33.33333), 53033005304 (UW/U-District, 40.54054), 53033005307 (UW/U-District, 68.14404)

sort <- data %>% 
  dplyr::filter(estimate==100) #27 census tracts 

table(data$county_name)
table(sort$county_name)
```

1. 93%: The region's average share of households without internet access
2. 27: Number of census tracts where all households have internet access
3. 74%: The lowest internet access census tract in the region, outside of the UW campus and U-District

\

#### Insights & Analysis

* King and Kitsap counties have the highest share of households with internet access (94%), followed by Snohomish (93%), and Pierce (92%)
* Kitsap County has the highest share of census tracts where all households have internet access (7%), compared to 3% of the census tracts in King and Pierce, and 2% in Snohomish 
...
# set folder structure
rda <- 'h01-internet-access-data.rda'
...
num_colors = 2
color_rev = TRUE
title = "Internet Access by Community"
...
# set folder structure
file_name <- 'h01-internet-access-column'
...
# King: 0.973-0.867=0.106
# Kitsap: 0.970-0.890=0.08
# Pierce: 0.959-0.858=0.101
# Snohomish: 0.968-0.857=0.111

filter <- data_clean_column %>%
  filter(county!="Region",
         focus_type=="Disability_cat")

# King: 0.971-0.887=0.084
# Kitsap: 0.967-0.924=0.043
# Pierce: 0.951-0.905=0.046
# Snohomish: 0.965-0.896=0.069
...
1. 87%: The regional share of households with internet access for those who are below 200% of the poverty level
2. 7%: Non-English proficient households have access to internet 7% lower than English proficient households
3. 99%: Internet access for households with children (below 18 years old) is 99%, compared to 93% for households without children

\

#### Insights & Analysis

* Snohomish County is the only county in the region where households with people of color have higher access to internet when compared to white non-Hispanic households. The difference is still very small (1%)
* For households who are below 200% of the poverty level, the share of households with internet access is lowest in Snohomish and Pierce counties (86%), followed by King (87%), and Kitsap (89%). The largest difference between low income and other households is in Snohomish and King (11%), Pierce (10%), and Kitsap (8%)
* The largest difference in internet access between people with and without a disability is in King County (8%), Snohomish (7%), Pierce (5%), and Kitsap (4%)
* Households with older adults (65+) consistently have less access to internet compared to other households across the region, with differences ranging between 5-10%

\
\

# **ignore all info below**
Although the chart contains accurate information, data is only available for the most recent year. Therefore, the chart does not show any additional information from the chart above. The data call outs and insights/analysis have not been updated, so they can be disregarded.
...
subtitle = "Percent of households with access to internet, in 5-year spans between 2011 and 2021"
source = "U.S. Census Bureau, 2007-2011, 2011-2016, 2017-2021 American Community Survey 5-Year Public Use Microdata Sample"
...
# set folder structure
file_name <- 'h01-internet-access-line'
