...
#internet access does not have 2012 data so it needs to be filtered out for the line chart
data_clean <- data_clean %>% filter(data_year != '2012')
...
acs_data_per <- base_acs_data %>%
  dplyr::filter(variable == "S2801_C02_012")
...
#                            vars = c("ACCESSINET"))
# 
# reg_median <- psrc_pums_median(pums_data, stat_var = c("ACCESSINET"))
# # 
# # # add region median to county data
# county_reg_data <- county_data %>%
#   mutate(reg_estimate=reg_median$ACCESSINET_median)
# 
...
county_internet <- county_acs_data %>% 
  dplyr::filter(variable=="S2801_C01_012",
                name!="Region") %>% 
  dplyr::select(name, estimate) %>% 
  dplyr::rename(internet=estimate)
county_pop <- county_acs_data %>% 
  dplyr::filter(variable=="S2801_C01_001",
                name!="Region") %>% 
  dplyr::select(name, estimate) %>% 
  dplyr::rename(pop_internet=estimate)

county_merge <- merge(county_internet, county_pop,
                      by.x="name",
                      by.y="name",
                      all.x=TRUE)
county_data_simp <- county_merge %>% 
  dplyr::mutate(cnty_estimate=internet/pop_internet) %>% 
...
reg_merge <- merge(reg_internet, reg_pop,
                      by.x="name",
                      by.y="name",
                      all.x=TRUE)
reg_data <- reg_merge %>% 
  dplyr::mutate(reg_estimate=internet/pop_internet) %>% 
