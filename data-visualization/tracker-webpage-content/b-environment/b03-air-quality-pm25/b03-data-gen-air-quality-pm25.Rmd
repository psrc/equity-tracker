...
raw_data <- "pm25_2024.csv"

# set the variables
var_name <- "PM2.5 Pollution"
year <- 2024
years <- c(2024)
data <- 1 #number of datasets in raw_data list - to iterate through
```

**Air Quality: PM2.5 Pollution in the Region**

# Download data 
## Primary data 
These data come via request from the [Puget Sound Clean Air Agency](https://pscleanair.gov/154/Air-Quality-Data). PSCAA began processing the data in this way beginning with 2020 observations. The data are available at 2020 geographies. A CSV of the raw data, exported from a shapefile, has been saved in the corresponding Y drive project folder - folder address included above.
```{r}
...
```{r}
# Convert geoid20 field to string
df_raw_data$geoid20 <- as.character(df_raw_data$geoid20)
```
...
tract_indicator <- df_raw_data %>% # insert the name of your data set
  rename(variable_value = MEAN, # insert the name of your indicator variable
         geoid = geoid20) %>% # insert the name of your census tract or geography variable
  mutate(data_year = 2024) # create data year field
```
...
data_tract <- merge(tracts20.lyr, tract_indicator,
                    by.x="geoid20",
                    by.y="geoid", 
                    all.x=TRUE)
```
...
summary(data_tract$variable_value)
```
...
distrib <- rep(data_tract$variable_value) # this should be commented out if you are weighting your variable
...
data_tract_outliers <- data_tract %>%
  dplyr::filter(variable_value %in% outlier_values)

num_outliers <- nrow(data_tract_outliers) #21
```
\
...
pop_outliers <- as.numeric(sum(data_tract_risk_outliers$total_pop20, na.rm = T)) #104,553
# There are approx. 104,553 people (2020 population) in these outlier tracts - just for a broad estimate.
```
...
PM2.5 pollution data will be weighted by area in order to aggregate to the region/county level - these values will be referred to in the code and visualized on the map for regional context. 
...
county_tract_data <- data_tract %>%
  ...
  dplyr::select(county_name, variable_value, land_acres) %>% #to simplify table to necessary fields
  dplyr::group_by(county_name) %>%
  dplyr::mutate(cnty_estimate=(sum(variable_value*land_acres, na.rm =TRUE))/(sum(land_acres, na.rm=TRUE))) %>% #calculate weighted value by county
  dplyr::distinct(county_name, cnty_estimate)

# calculating data by region
reg_tract_data <- data_tract %>%
  st_drop_geometry() %>%  #to remove spatial component
  dplyr::select(variable_value, land_acres) %>% #to simplify table to necessary fields
  dplyr::mutate(reg_estimate=(sum(variable_value*land_acres, na.rm =TRUE))/(sum(land_acres, na.rm=TRUE))) %>% #calculate weighted value for region
...
data_equity_tracts <- merge(tract_indicator, filter(equity_tracts, data_year == 2023),
                            by=c("geoid"),
                            all.x=TRUE) %>% 
  select(-data_year.y) %>% 
  rename(data_year = data_year.x) %>% 
  # merge with tract layer to grab land acres for weighting
  merge(., select(tracts20.lyr, geoid20, land_acres),
...
data_tract <- data_equity_pivot %>%
  ...
  mutate(tract_weight = variable_value*land_acres) %>%
  group_by(county, data_year, equity_group, quintile) %>% # county field includes region
  summarise(tracts = length(geoid),
            total_variable = sum(tract_weight, na.rm = TRUE),
            total_acres = sum(land_acres, na.rm = TRUE),
            .groups = "drop") %>%
  mutate(variable_value = total_variable/total_acres)
...
data_tract_comb <- rbind(low_lowmed, #2
                         other_data) #148

# nrow(data_tract_comb) #150 - this number should be smaller but there are duplicates - low and low medium present in data set because of aggregation/calculations in previous steps - so the extras need to be removed and replaced with averaged values
