...
# summary(data_tract$estimate_prelog) # max is 568196.6, so rounding to 600,000 makes sense
max_value <- 600000 # this value will be the max value in the legend
...
# psrc_palette <- leaflet::colorNumeric(palette = psrc_purple_plus,
#                                       domain = c(min(log(1+tract_data_plus_zero),na.rm = TRUE),max_value, NA)) # this is the value field

# set the variable
var_name <- "Toxic Release <br> Exposure"
labels <- sprintf(
  ...
  paste("Tract value: ", prettyNum((round(data_tract$estimate_prelog, digits=0)),
                  big.mark = ",")),
...
tract_map <- leaflet() %>%
  ...
  addPolygons(data=data_tract,
              ...
              # fillColor = psrc_palette(data_tract$estimate_prelog), #tract_data_plus_zero
  ...
  addLegend_decreasing(pal = psrc_palette,
                       # values = c(min(log(1+tract_data_plus_zero),na.rm = TRUE),max_value, NA),
                       # values = c(min(data_tract$estimate_prelog,na.rm = TRUE),max_value, NA),
...
# check maximum value of data
# summary(data_tract$estimate) # max is 13.25, so rounding to 14 makes sense
# summary(log(1+data_tract$estimate_prelog))
max_value <- 14 # this value will be the max value in the legend
...
labels <- sprintf(
  "Census tract: <em>%s</em><br/><strong>%s</strong><br/><body>%s</body><br/><body>%s</bodyD>", data_tract$geoid20, # the code in <> makes the geoid value italic 
  paste("Tract value: ", prettyNum((round(data_tract$estimate_prelog, digits=1)))),
  paste("Region value: ", prettyNum((round(data_tract$reg_estimate, digits=1)))),
...
num_colors = 5
color_rev = FALSE
title = "Toxic Release Exposure"
subtitle = "exposure for people within a 2 mile buffer of on-site releases"
...
# set variables
df = data_clean
x = "data_year_yr" # different from variables for facet column
