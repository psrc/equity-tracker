...
blockgroup_shares <- get_acs(geography="block group", variables=vars, state=53,
...
blockgroup_shares[, (quintile_vars):=lapply(.SD, label_quintile), .SDcols=share_vars]              # Add quintile value
setnames(blockgroup_shares, old=c("GEOID","Year"), new=c("geoid", "data_year"))

#filename <- paste0(getwd(),"/equity_shares.csv")
#fwrite(equity_shares, filename)                                                                   # Write to file

sockeye_connection <- elmer_connect()
table_id <- Id(schema = "stg", table = "blockgroup_shares")
dbWriteTable(sockeye_connection, table_id, blockgroup_shares, overwrite = TRUE)                    # Write staging table
merge_sql <- paste("MERGE INTO equity.blockgroup_shares WITH (HOLDLOCK) AS target",                # Insert new data
                   "USING stg.blockgroup_shares AS source",
                   "ON target.data_year=source.data_year AND target.geoid=source.geoid",
                   "WHEN NOT MATCHED BY TARGET THEN INSERT (", paste0(colnames(blockgroup_shares), collapse=", "),")",
                   "VALUES (", paste0("source.", colnames(blockgroup_shares), collapse=", "), ");")
run_query(sockeye_connection, merge_sql)                                                           # Execute the query
run_query(sockeye_connection, "DROP TABLE stg.blockgroup_shares")                                  # Clean up
