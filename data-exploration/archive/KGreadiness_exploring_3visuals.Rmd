...
data_full <- pums_ospi_elmer %>% 
  ...
  # dplyr::mutate(data_year_yr=lubridate::as_date(paste0(data_year,"-01-01")))
```
...
facet_line_by_geography <- function(df, geography, focus.text){
  ...
  static_facet_line_chart(t=data_clean_geo,
                          x="data_year_yr", y="fact_value",
                          # x=lubridate::data_year_yr(x), y="fact_value",
                          fill="vulnerability", facet="focus_type_ord",
                          color="psrc_light",
                          est ="percent",
                          dform = "%Y",
...
# save(data_clean, file = "T:/2023June/Mary/EquityTracker/data/kgreadiness.rda")
```
\
\
...
# merge kg readiness data with population data
...
labels <- sprintf(
  ...
  kg_schdistrict_22$lea_name, paste(prettyNum(round(kg_schdistrict_22$Percent_num), digits=2),'%')) %>% 
...
website_dev_folder <- paste0("//WEB/website_data/equity-tracker-webpages/economy/kg-readiness","/")
...
# 
# map_kg <- create_psrc_map(lyr=kg_schdistrict_22,
#                           lyr_data_field=kg_schdistrict_22$Percent_num,
#                           legend_title = "Kindergarten readiness",
#                           legend_subtitle = "6 of 6 domains",
#                           psrc_col_pal=psrc_purple_plus)

# fix the legend NA placement (https://github.com/rstudio/leaflet/issues/615)
# css_fix <- "div.info.legend.leaflet-control br {clear: both;} html * {font-family: Poppins !important;}" # CSS to correct spacing and font family
# html_fix <- htmltools::tags$style(type = "text/css", css_fix)  # Convert CSS to HTML
# map_kg %<>% htmlwidgets::prependContent(html_fix)      # Insert into leaflet HTML code
...
### Copy files from GitHub > website folder
```{r, include=FALSE, eval=FALSE}
current.folder <- "C:/Users/mrichards/Documents/GitHub/equity-tracker/data-exploration/KGreadiness_exploring_3visuals_files/figure-html"
new.folder <- "//WEB/website_data/equity-tracker-webpages/economy/kg-readiness"
