...
unique(pums_ospi_elmer$data_year_yr)
```
...
data_indicator <- pums_ospi_elmer %>% 
 ...
 # group_by(indicator_attribute, vulnerability, indicator_type, focus_type, focus_attribute, data_year_yr, county, fact_value, margin_of_error)%>%
...
data_full <- data_indicator %>% 
  ...
  summarise(fv = sum(fact_value), moe = moe_sum(margin_of_error, estimate = fact_value, na.rm = TRUE))
...
facet_by_geography <- function(geography, focus.text){
  # filter data
  data_clean_geo <- data_clean_oneplus %>% 
  ...
  static_facet_column_chart(t=data_clean_geo,
                            ...
                            source="U.S. Census Bureau, American Community Survey (ACS) 5-Year Public Use Microdata Sample (PUMS)")
 #   geom_text(aes(label = paste0(fv*100,"%")), 
...
dif_groups <- function(df, geography, group, category){
  region_btgrp_dif <- data_clean_oneplus %>% 
...
equity_group_facet_line <- function(df, focus.type, focus.text){
  # filter data
  data_clean_filter <- data_clean_oneplus %>% 
...
data_clean_df <- data_clean_oneplus %>% 
...
```{r}
data_clean_region <- data_clean_oneplus %>% 
  dplyr::filter(county=="Region")
