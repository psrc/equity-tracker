---
title: "Equity Quintiles by Census Tracts"
...
\
\
In addition to these equity groups, population, population density, and census tract information is also viewable for context. The tract ids can be searched by their geoid.

# Data
Data are from based on the 2020 reference year at the census tract geographic scale.
...
```{r, include=FALSE}
head(equity_tracts)
num_row <- nrow(equity_tracts) #919 tracts in the region with equity quintile information

unique(equity_tracts$data_year) #2020
equity_tracts_20 <- equity_tracts %>%
  filter(data_year == 2020)
# num_row <- label_comma()(nrow(equity_tracts_20)) #919 tracts (2020) in the region with equity quintile information
```
There are `r num_row` tracts (2020) in the region with equity data. 
\

* **2020 Tracts (PSRC Portal)**
...
tracts.lry<-st_read(tracts.url)
# head(tracts.lry)
# plot(tracts.lry$geometry)
nrow(tracts.lry) #919

# join base 2020 geographic data to PUMS equity quintile data
equity_quintile_tract20 <- merge(tracts.lry, equity_tracts_20,
                              by.x="geoid20",
                              by.y="geoid", 
                              all.x=TRUE)

# head(equity_quintile_tract20)
# class(equity_quintile_tract20) #sf and data.frame
# plot(equity_quintile_tract20$geometry)

pop_2020 <- equity_quintile_tract20 %>% 
...
equity_quintile_tract20 <- equity_quintile_tract20 %>% 
  mutate(sqmi_aland20=aland20*3.58701e-8) %>% 
  mutate(pop_den_sqmi=round((total_pop20/(land_acres*0.0015625)),0))

summary(equity_quintile_tract20$pop_den_sqmi)
boxplot(equity_quintile_tract20$pop_den_sqmi)

# refactoring variables - ordering quintiles: low, low medium, medium, medium high, high
equity_quintile_tract20$poc_quintile <- factor(equity_quintile_tract20$poc_quintile,
                                            levels = c('Low','Low Medium','Medium', 'Medium High','High'))
equity_quintile_tract20$income_quintile <- factor(equity_quintile_tract20$income_quintile,
                                               levels = c('Low','Low Medium','Medium', 'Medium High','High'))
equity_quintile_tract20$disability_quintile <- factor(equity_quintile_tract20$disability_quintile,
                                                   levels = c('Low','Low Medium','Medium', 'Medium High','High'))
equity_quintile_tract20$youth_quintile <- factor(equity_quintile_tract20$youth_quintile,
                                              levels = c('Low','Low Medium','Medium', 'Medium High','High'))
equity_quintile_tract20$older_quintile <- factor(equity_quintile_tract20$older_quintile,
                                              levels = c('Low','Low Medium','Medium', 'Medium High','High'))
equity_quintile_tract20$lep_quintile <- factor(equity_quintile_tract20$lep_quintile,
                                            levels = c('Low','Low Medium','Medium', 'Medium High','High'))
...
population_pal <- leaflet::colorNumeric(palette = rev(colorspace::sequential_hcl(30,"YlGn")), 
                                        domain =  equity_quintile_tract20$total_pop20)
# popdensity_pal <- leaflet::colorNumeric(palette = "Blues", 
#                                         domain =  equity_quintile_tract20$pop_den_sqmi)
popdensity_pal <-  leaflet::colorQuantile(palette = colorspace::sequential_hcl(5, "Emrld"),
                                     domain = equity_quintile_tract20$pop_den_sqmi,
                                     n = 5, reverse = TRUE)
POC_pal <- leaflet::colorFactor(palette="Purples",
                                equity_quintile_tract20$poc_quintile)
income_pal <- leaflet::colorFactor(palette="Purples",
                                   domain = equity_quintile_tract20$income_quintile)
disability_pal <- leaflet::colorFactor(palette="Purples",
                                       domain = equity_quintile_tract20$disability_quintile)
youth_pal <- leaflet::colorFactor(palette="Purples",
                                  domain = equity_quintile_tract20$youth_quintile)
older_pal <- leaflet::colorFactor(palette="Purples",
                                  domain = equity_quintile_tract20$older_quintile)
LEP_pal <- leaflet::colorFactor(palette="Purples",
                                domain = equity_quintile_tract20$lep_quintile)


label_bg <- stringr::str_c(
  "Tract: ", as.character(equity_quintile_tract20$geoid20))
label_popdens <- stringr::str_c(
  "Pop. Density (per sqmile): ", formatC(equity_quintile_tract20$pop_den_sqmi, 
                                  big.mark = ",", 
                                  format = "d"))
# label_pop <- stringr::str_c(
#   "Population: ", formatC(equity_quintile_tract20$total_pop20, 
#                                   big.mark = ",", 
#                                   format = "d"))
...
equity_quintile_map <- leaflet() %>%
  ...
  addLayersControl(#baseGroups = "Census Tracts",
                   overlayGroups = c("People of Color",
                                     "Low Income",
                                     "Disability",
                                     "Youth",
                                     "Older Adults",
  ...
  # polygon layers
  # addPolygons(data=equity_quintile_tract20,
  #             color = population_pal(equity_quintile_tract20$total_pop20),
  #             stroke=FALSE, 
  #             smoothFactor = 0.2,
  #             fillOpacity = 1.0,
  #             group = "Population",
  ...
  addPolygons(data=equity_quintile_tract20,
              color = popdensity_pal(equity_quintile_tract20$pop_den_sqmi),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 1.0,
  ...
  addPolygons(data=equity_quintile_tract20,
              ...
              highlightOptions = highlightOptions(
                ...
                bringToFront = TRUE, sendToBack = TRUE)) %>% 
  addPolygons(data=equity_quintile_tract20,
              fillColor = POC_pal(equity_quintile_tract20$poc_quintile),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 1.0,
              ...
              label = ~poc_quintile) %>%
  addPolygons(data=equity_quintile_tract20,
              fillColor = income_pal(equity_quintile_tract20$income_quintile),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 1.0,
              group = "Low Income",
  ...
  addPolygons(data=equity_quintile_tract20,
              fillColor = disability_pal(equity_quintile_tract20$disability_quintile),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 1.0,
              group = "Disability",
  ...
  addPolygons(data=equity_quintile_tract20,
              fillColor = youth_pal(equity_quintile_tract20$youth_quintile),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 1.0,
              group = "Youth",
  ...
  addPolygons(data=equity_quintile_tract20,
              fillColor = older_pal(equity_quintile_tract20$older_quintile),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 1.0,
              group = "Older Adults",
  ...
  addPolygons(data=equity_quintile_tract20,
              fillColor = LEP_pal(equity_quintile_tract20$lep_quintile),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 1.0,
  ...
  leaflet::addLegend(pal = POC_pal,
                     values = equity_quintile_tract20$poc_quintile,
                     position = "bottomleft",
                     title = "People of Color",
                     group="People of Color",
                     opacity = 0.8) %>% 
  leaflet::addLegend(pal = income_pal,
                     values = equity_quintile_tract20$income_quintile,
                     position = "bottomleft",
                     title = "Low Income",
                     group = "Low Income",
                     opacity = 0.8) %>%
  leaflet::addLegend(pal = disability_pal,
                     values = equity_quintile_tract20$disability_quintile,
                     position = "bottomleft",
                     title = "Disability",
                     group = "Disability",
                     opacity = 0.8) %>%
  leaflet::addLegend(pal = youth_pal,
                     values = equity_quintile_tract20$youth_quintile,
                     position = "bottomleft",
                     title = "Households with Youth",
                     group = "Youth",
                     opacity = 0.8) %>%
  leaflet::addLegend(pal = older_pal,
                     values = equity_quintile_tract20$older_quintile,
                     position = "bottomleft",
                     title = "Older Adults",
                     group = "Older Adults",
                     opacity = 0.8) %>%
  leaflet::addLegend(pal = LEP_pal,
                     values = equity_quintile_tract20$lep_quintile,
                     position = "bottomleft",
                     title = "Limited English Proficiency",
                     group = "Limited English Proficiency",
                     opacity = 0.8) %>%
  leaflet::addLegend(pal = popdensity_pal,
                     values = equity_quintile_tract20$pop_den_sqmi,
