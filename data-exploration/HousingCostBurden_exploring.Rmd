...
indicator_measure <- 'housing_cost_burden'
# set up a variable for titles
indicator_title <- 'Housing Cost Burden'
```
...
data_clean$indicator_attribute2 <- factor(data_clean$indicator_attribute,
...
data_clean_fiftyplus <- data_clean %>% 
  filter(indicator_attribute=="Greater than 50 percent")
data_clean_between <- data_clean %>% 
  filter(indicator_attribute=="Between 30 and 50 percent")
...
qplot(data_year_yr, fact_value, 
      ...
      data = data_clean_fiftyplus, 
      main = paste0(indicator_title, ": Region, greater than 50 percent")) + theme(axis.title.x = element_blank(),
                                                          axis.title.y = element_blank()) + ylim(0, 1)

qplot(data_year_yr, fact_value, 
      colour = county, shape = focus_type_ord, 
      data = data_clean_between, 
      main = paste0(indicator_title, ": Region, between 30 and 50 percent")) + theme(axis.title.x = element_blank(),
                                                          axis.title.y = element_blank()) + ylim(0, 1)
...
data_clean11 <- data_clean_fiftyplus %>% 
  filter(data_year==2011)
data_clean16 <- data_clean_fiftyplus %>% 
  filter(data_year==2016)
data_clean21 <- data_clean_fiftyplus %>% 
  filter(data_year==2021)
```
...
facet_by_geography <- function(geography, focus.text){
  # filter data
  data_clean_geo <- data_clean_fiftyplus %>% 
...
dif_groups <- function(geography, group, category){
  region_btgrp_dif <- data_clean_fiftyplus %>% 
...
equity_group_facet_line <- function(focus.type, focus.text){
  # filter data
  data_clean_filter <- data_clean_fiftyplus %>% 
...
dif_groups <- function(group, category){
  ...
  btgrp_dif <- data_clean_fiftyplus %>% 
    dplyr::filter(focus_type==group) %>% 
    group_by(data_year) %>% 
    mutate(dif = ifelse(focus_attribute!=category, fact_value - fact_value[focus_attribute == category], 0)) %>% 
    dplyr::filter(dif!=0)
...
data_clean_df <- data_clean_fiftyplus %>% 
...
```{r}
data_clean_region <- data_clean_fiftyplus %>% 
  dplyr::filter(county=="Region")
