...
# save(data_clean, file = "T:/2023June/Mary/EquityTracker/data/medincome.rda")
```
\
\
...
```{r, include=FALSE, eval=FALSE}
# current.folder <- "Y:/Equity Indicators/tracker-webpage-content/economy/candidates/"
...
```{r}
# source('C:/Users/mrichards/Documents/GitHub/psrc.map/map_region.R')
# 
# psrc_purple_plus <- append(psrc_colors$purples_inc, '#FFFFFF', after=0)
# 
# map_med_income <- create_psrc_map(lyr=medinc_tract20,
#                                   lyr_data_field=medinc_tract20$estimate,
#                                   legend_title = "Median household income",
#                                   legend_subtitle = "2021 $USD",
...
region_graphic <- facet_line_by_geography(data_clean, "Region", ": Region")

region_graphic + coord_cartesian(ylim=c(0,160000)) + theme(strip.text.x = element_text(size = 11), )
# save(data_clean, file = "T:/2023May/Mary/EquityTracker/data/data_clean.rda")
``` 
...
facet_line_by_geography(data_clean_2yr, "Region", ": Region") + coord_cartesian(ylim = c(10000, 160000))
```
...
data_clean21 %>% 
  group_by(county_ord, vulnerability) %>% 
  dplyr::mutate(focus_type_ord= str_wrap(focus_type_ord, 4)) %>%
  e_charts(focus_type_ord) %>% 
  e_bar(fact_value) %>%
  e_bar(vulnerability) %>%
  # e_facet(rows = 3, cols=2,
  #         legend_pos = "top") %>% 
  # e_legend_toggle_select("King", btn = "king_btn") %>%
  # e_button("king_btn", "King", position="bottom") %>%
  # e_legend_toggle_select("Kitsap", btn = "kitsap_btn") %>%
  # e_button("kitsap_btn", "Kitsap", position="bottom") %>%
  # e_legend_toggle_select("Pierce", btn = "pierce_btn") %>%
  # e_button("pierce_btn", "Pierce", position="bottom") %>%
  # e_legend_toggle_select("Snohomish", btn = "snoho_btn") %>%
  # e_button("snoho_btn", "Snohomish", position="bottom") %>%
  e_title("test") %>%
  e_legend_unselect("King") %>% 
  e_legend_unselect("Kitsap") %>% 
  e_legend_unselect("Pierce") %>% 
  e_legend_unselect("Snohomish") %>% 
  # Hide Legend
  e_legend(show=TRUE) %>% 
  e_tooltip(formatter = e_tooltip_item_formatter("currency")) %>% 
  e_x_axis(axisLabel = list(interval = 0, rotate = 45)) %>% 
  e_y_axis(formatter = e_axis_formatter("currency")) #only works when legend is TRUE and buttons are turned off
  # e_flip_coords()
  # e_tooltip(formatter='{a}: {@fact_value}')

  
  
# save(data_clean, file = "T:/2023June/Mary/temp/forCraig/data_clean.rda")
```
