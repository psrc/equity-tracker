...
#try msa/md identifiers_
msa_lookup <- data.frame(msa_md_name=c("King/Snohomish","Kitsap","Pierce"),
                         fips=paste0(c("42644","14740","45104")))%>%
  rbind(c(msa_md_name="Region",
          fips=c(paste0(.$fips, collapse=",")))) %>%
  setDT() %>% setkey(fips)
...
#msa version
hmda_url_builder_msa <- function(dyear,
                             actions_taken,
                             counties="Region",
                             races=NULL,
                             ethnicities=NULL,
  ...
  url <- paste0("https://ffiec.cfpb.gov/v2/data-browser-api/view/aggregations?counties=",
                msa_lookup[msa_md_name==msa/md, fips],
                "&years=", dyear, "&actions_taken=", actions_taken)                               # Multiple years not allowed here, because API sums them unless requested separately
...
# Primary hmda function for County and Equity Tracker --------------------------
...
# Primary hmda function for MSA/MD and Equity Tracker --------------------------

get_hmda_msa <- function(dyears, races=races_vector){
  mixr <- expand.grid(dyear=dyears, actions_taken=c(1,3), races=races)                             # Notice the API aggregates all specified items
  rs   <- data.frame(
    mapply(hmda_url_builder_msa,
           dyear=mixr$dyear,                                                               # So years, details must be called individually
           actions_taken=mixr$actions_taken,
           races=mixr$races),
    mixr$dyear) %>%
    transpose() %>% lapply(hmda_api_gofer) %>% rbindlist() %>%
    setDT() %>% setnames("msa/md", "fips") %>% setkey(fips)
  rs %<>% .[msa_lookup, msa/md:=msa_md_name] %>%
    dcast(year + county + races ~ actions_taken, value.var="count") %>%
    .[, denial_share:=(`3`/`1`)] %>% .[, c("3","1"):=NULL]
  return(rs)
}
...
#denial_rate_x_race_msa <- get_hmda_msa(2018:2021)
