---
title: "3.) Voter Turnout: 2012-2024"
...
\
\
For the 2024 election data, we are calculating voter turn out based on earlier exploration (testing 4 methods - 1-voter-turnout-exploration-24.qmd). The numerator is calculated by weighting (or splitting) the ballots_cast per precinct by the percent_total_population (distribution of the population distributed among the precinct-tract pairs). The percent_total_population values are based on the parcel_year, so they are weighted by the ACS 5y population values for the corresponding base year - for this election year, it is the 2023 population numbers. These values are then summed up to the tract level. The denominator is calculated by using OFM Small Area Estimates Program (SAEP) 2024 total population estimates (TRACT), weighted by the ACS 2024 5y adult 18+ population/total population ratio (TRACT). The denominator represents the population eligible to vote. These data will be calculated based on 2020 census geographies.
\
\
For the 2020 election data, the numerator is calculated using the same process described for the 2024 data - by weighting (or splitting) the ballots_cast per precinct by the percent_total_population, then aggregating to the tract level. The percent_total_population values are based on the 2018 base year and therefore the 2018 ACS 5y population numbers. The denominator is calculated by using OFM Small Area Estimates Program (SAEP) 2020 total population estimates (TRACT), weighted by the 2020 decennial census adult 18+ population/total population ratio (TRACT). This is the same approach as used for 2024, but using decennial census instead of ACS 2024 5y estimates. These data will be calculated based on 2020 census geographies.
\
\
For the 2016 election data, the numerator is calculated using the same process described for the 2024 data - by weighting (or splitting) the ballots_cast per precinct by the percent_total_population, then aggregating to the tract level. The percent_total_population values are based on the 2014 base year and therefore the 2014 ACS 5y population numbers. The denominator is calculated by using OFM Small Area Estimates Program (SAEP) 2016 total population (group quarters + household population) block estimates (2020 vintage) in 2010 census geographies, aggregated to tract, and weighted by the ACS 2016 5y adult 18+ population/total population ratio (TRACT). These data will be calculated based on 2010 census geographies.
\
\
For the 2012 election data, the numerator is calculated using the same process described for the 2024 data - by weighting (or splitting) the ballots_cast per precinct by the percent_total_population, then aggregating to the tract level. The percent_total_population values are based on the 2014 base year and therefore the 2014 ACS 5y population numbers. The denominator is calculated by using OFM Small Area Estimates Program (SAEP) 2012 total population (group quarters + household population) block estimates (2020 vintage) in 2010 census geographies, aggregated to tract, and weighted by the ACS 2012 5y adult 18+ population/total population ratio (TRACT). These data will be calculated based on 2010 census geographies.


*DATA CAVEATS:* 

* For this exercise, it is assumed that the eligible population most closely aligns with the adult 18+ population. Although this may include individuals who are not able to legally vote (non-citizens), the alternative is using the number of registered voters (available at voting precincts), which is a subset of the adult 18+ population who are already engaged enough to have registered. While using the general 18+ population as a denominator may result in slightly lower voter turnout values (based on our working definition), using registered voters as the denominator would result in inflated voter turnout values.
...
* To calculate the numerator (ballots_cast), we use 'percent_total_population' since there is no closer measure available when splitting precincts that overlap with more than one tract (other 4 options: group quarters, household population, housing units, occupied housing units). The 'percent_total_population' represents the total population, while the denominator is an estimation of the adult population.


# I) 2024 (OFM SAEP 2024; ACS 5y 2024)
In 2020 census geographies. Changing from 2023 ACS 5y to 2024 ACS 5y data (February 2026 - Census just released 5y estimates). 
...
tract_population <- ofm_psrc_20 %>% 
  left_join(age_18over_total_24, by= c("GEOID20"="GEOID"))

# weighting the OFM population estimates by ACS 5y 18+/total proportion, 
...
# weighting the OFM population estimates by ACS 5y census 18+/total proportion, 
...
tract_map <- leaflet() %>%
  ...
  leaflet::addProviderTiles("CartoDB.VoyagerOnlyLabels",
                            options = leaflet::leafletOptions(pane = "maplabels"),
                            group = "Labels") %>%
  leaflet::addLayersControl(baseGroups = c("2012", "2020", "2024"),
